...
```
SpaceDial/
├── manifest.json          # Extension manifest (MV3, v4.9.0)
├── newtab.html            # Main new tab page
├── newtab.js              # All UI logic + settings
├── newtab.css             # All styles
├── console-capture.js     # Early console capture (before Three.js)
...
├── scan-wallpapers.ps1    # Dev utility — scan wallpaper files
...
│   │   ├── sun.jpg
│   │   └── theme.json
│   ├── galaxy/            # Galaxy theme — 3D planets + raymarched black hole
│   │   ├── galaxy-theme.js    # Raymarching shader (GLSL), bloom post-processing
│   │   ├── blackhole-sgra.jpg # Sagittarius A* reference texture
│   │   ├── blackhole-reference.png
...
| Galaxy body | Appearance | Earth | `milkyway` (panorama), `earth` (3D globe), `blackhole` (raymarched accretion disk with bloom), `mercury`, `venus`, `mars`, `jupiter`, `saturn`, `uranus`, `neptune` (3D spheres with real textures) — shown only when theme is Galaxy |
...
- `console-capture.js` loads before Three.js to capture all console warnings/errors for the developer panel.
- Galaxy theme black hole uses a self-contained bloom pipeline (threshold + Gaussian blur + composite) via WebGL render targets — no external post-processing libraries.
