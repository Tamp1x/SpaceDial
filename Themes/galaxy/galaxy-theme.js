...
const GalaxyTheme = (() => {
  ...
  let animStartTime = 0;
  let blackHoleMesh = null, blackHoleMat = null;
  let currentPlanet = 'earth';
  let spinSpeed = 0.4;
  let dragHandlers = null;
  let isDragging = false;
  let dragStartX = 0, dragStartY = 0, dragStartRotY = 0, dragStartRotX = 0;
  let bloomComposer = null;
  let blackPlanetCanvas = null;
  let blackPlanetCtx = null;
  let blackPlanetStars = [];
  let blackPlanetActive = false;
  ...
  const PLANETS = {
    ...
    blackplanet: { scale: 1.3, speed: 0.0, ring: false, unlit: true, blackPlanet: true, zoom: 6.0 },
    blackhole: {
  ...
  function createStarField(twinkle = true) {
    const count = 900;
    ...
    points.userData.twinkle = twinkle;
    return points;
  }
  ...
  function buildPlanet(planetId) {
    ...
    if (cfg.blackPlanet) {
      blackPlanetActive = true;
      disposeAll();
      if (renderer) { renderer.dispose(); renderer = null; }
      scene = null; camera = null; planetGroup = null; starMesh = null; ringMesh = null;
      removeDragControls();

      if (!blackPlanetCanvas) {
        blackPlanetCanvas = document.createElement('canvas');
        blackPlanetCanvas.style.position = 'fixed';
        blackPlanetCanvas.style.inset = '0';
        blackPlanetCanvas.style.zIndex = '5';
        blackPlanetCanvas.style.pointerEvents = 'none';
        document.body.appendChild(blackPlanetCanvas);
      }
      blackPlanetCanvas.width = window.innerWidth;
      blackPlanetCanvas.height = window.innerHeight;
      blackPlanetCanvas.style.display = 'block';
      blackPlanetCtx = blackPlanetCanvas.getContext('2d');

      blackPlanetStars = [];
      const w = blackPlanetCanvas.width;
      const h = blackPlanetCanvas.height;
      const count = Math.min(200, Math.max(80, Math.round((w * h) / 8000)));
      for (let i = 0; i < count; i++) {
        blackPlanetStars.push({
          x: Math.random() * w,
          y: Math.random() * h,
    ...
    blackPlanetActive = false;
    if (blackPlanetCanvas) blackPlanetCanvas.style.display = 'none';
  ...
  function animate(time) {
    if (!blackPlanetActive && (!renderer || !scene || !camera)) { animId = null; return; }
    animId = requestAnimationFrame(animate);
    if (!animStartTime) animStartTime = time;
    const elapsed = (time - animStartTime) / 1000;

    if (blackPlanetActive && blackPlanetCtx) {
      const w = blackPlanetCanvas.width;
      const h = blackPlanetCanvas.height;
      const cx = w / 2;
      const cy = h / 2;
      const r = Math.min(w, h) * 0.28;
      const ctx = blackPlanetCtx;

      ctx.clearRect(0, 0, w, h);
      ctx.fillStyle = '#000';
      ctx.fillRect(0, 0, w, h);

      for (const star of blackPlanetStars) {
        ctx.beginPath();
    ...
    if (starMesh?.material) {
      starMesh.rotation.y = elapsed * 0.002;
      if (starMesh.userData.twinkle) {
        starMesh.material.opacity = 0.7 + Math.sin(elapsed * 0.8) * 0.2;
      }
    }
  ...
  function handleResize() {
    if (blackPlanetActive && blackPlanetCanvas) {
      blackPlanetCanvas.width = window.innerWidth;
      blackPlanetCanvas.height = window.innerHeight;
      return;
    }
  ...
  return {
    init(canvas, planetId) {
      ...
      if (PLANETS[currentPlanet]?.blackPlanet) {
        blackPlanetActive = true;
        if (!blackPlanetCanvas) {
          blackPlanetCanvas = document.createElement('canvas');
          blackPlanetCanvas.style.position = 'fixed';
    ...
    setPlanet(planetId) {
      ...
      if (PLANETS[planetId].blackPlanet) {
    ...
    destroy() {
      ...
      if (renderer) { renderer.dispose(); renderer = null; }
      if (blackPlanetCanvas) {
        blackPlanetCanvas.style.display = 'none';
        blackPlanetCanvas.remove();
        blackPlanetCanvas = null;
        blackPlanetCtx = null;
        blackPlanetStars = [];
      }
      blackPlanetActive = false;
